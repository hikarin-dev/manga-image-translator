...
from .xpos_relative_position import XPOS, fixed_pos_embedding, apply_rotary_pos_emb
...
class Model48pxExpOCR(OfflineOCR):
    _MODEL_MAPPING = {
        'model': {
    ...
    async def _load(self, device: str):
        ...
        # EXPERIMENTAL (48px_exp): compile the tensors-only decoder step (see OCR._step_math —
        # the per-character hot path: 5 decoder layers of norm / xpos-attention / FF = dozens
        # of tiny kernels per generated character; Inductor fuses the pointwise ops, shrinking
        # the host gaps that idle the GPU, plan §3.1). The step fn takes NO ints — the XPOS
        # tables that depended on `step` are precomputed at load and sliced eagerly — so
        # dynamic=True yields a handful of graphs (size-0/1 specializations, then symbolic),
        # all compiled here via warmup so no real page pays the JIT stall. Default mode (no
        # CUDA graph capture). suppress_errors: any Dynamo/Triton failure falls back to eager
        # per-graph (correct, just unaccelerated).
        self.model._step_c = None
        if self.use_gpu:
            try:
                import torch._dynamo as _dynamo
                _dynamo.config.suppress_errors = True
                _dynamo.config.cache_size_limit = max(32, getattr(_dynamo.config, 'cache_size_limit', 8))
                self.model.build_xpos_tables()
                self.model._step_c = torch.compile(self.model._step_math, dynamic=True)
                with torch.no_grad():
                    for n, w, s in ((1, 64, 12), (2, 96, 16), (3, 160, 20)):
                        dummy = torch.zeros(n, 3, 48, w, device=device)
                        self.model.infer_beam_batch_tensor(dummy, [w] * n, beams_k=5, max_seq_length=s)
            except Exception as e:
                self.model._step_c = None
                self.logger.warning(f'[48px_exp] torch.compile unavailable, using eager step: {e}')
...
# ── EXPERIMENTAL (48px_exp): torch.compile-friendly decoder step ─────────────────────────────
# The stock per-character step passes a Python int `step` into slicing and into XPOS (whose
# torch.arange bounds depend on it), so Dynamo re-specializes per step value → a recompile
# storm. Fix: precompute the XPOS (sin, cos, scale) tables for every (length, offset) the beam
# can ever use — with the exact same formula, once, at load — and compile a step function that
# takes ONLY tensors. The growing KV length is then a tensor dimension (symbolized cleanly by
# dynamic=True) and the int-indexed table slicing + cache write stay eager, outside the graph.
# Same math in the same order → same OCR text; guarded by the 48px-vs-48px_exp equivalence gate.

def _xpos_tables(xpos: XPOS, length: int, offset: int, downscale: bool):
    """Exactly XPOS.forward minus the final apply_rotary_pos_emb: returns (sin, cos, scale)."""
    min_pos = -(length + offset) // 2
    max_pos = length + offset + min_pos
    scale = xpos.scale ** torch.arange(min_pos, max_pos, 1).to(xpos.scale).div(xpos.scale_base)[:, None]
...
def _attn_with_tables(mod: 'XposMultiheadAttention', query, key, value, key_padding_mask,
                      q_sin, q_cos, q_scale, k_sin, k_cos, k_scale):
    """XposMultiheadAttention.forward with the XPOS module replaced by precomputed tables.
    Line-for-line the same ops in the same order (attn_mask/need_weights paths dropped —
    both call sites pass neither)."""
    bsz, tgt_len, embed_dim = query.size()
    key_bsz, src_len, _ = key.size()
    ...
    k = k.view(bsz, src_len, mod.num_heads, mod.head_dim).transpose(1, 2)
    v = v.view(bsz, src_len, mod.num_heads, mod.head_dim).transpose(1, 2)
    q = q.reshape(bsz * mod.num_heads, tgt_len, mod.head_dim)
    k = k.reshape(bsz * mod.num_heads, src_len, mod.head_dim)
    v = v.reshape(bsz * mod.num_heads, src_len, mod.head_dim)

    k = apply_rotary_pos_emb(k, k_sin, k_cos, k_scale)
    q = apply_rotary_pos_emb(q, q_sin, q_cos, q_scale)

    attn_weights = torch.bmm(q, k.transpose(1, 2))

    if key_padding_mask is not None:
        attn_weights = attn_weights.view(bsz, mod.num_heads, tgt_len, src_len)
        attn_weights = attn_weights.masked_fill(
            key_padding_mask.unsqueeze(1).unsqueeze(2).to(torch.bool),
            float("-inf"),
        )
...
class OCR(nn.Module):
    ...
    # EXPERIMENTAL (48px_exp): precompute every XPOS table the beam can use. q tables depend on
    # (length=1, offset=step, downscale=False); self-attn k tables on (length=step+1, offset=0,
    # downscale=True). Both are functions of `step` alone → S rows each, built once with the
    # exact original formula (same device/dtype/ops ⇒ identical bits). Cross-attn k tables
    # depend on the chunk's memory width → computed once per chunk in infer_beam_batch_tensor.
    def build_xpos_tables(self, max_steps: int = 256):
        xpos = self.decoders[0].self_attn.xpos  # all attn modules share (head_dim, scale_base)
        d = xpos.scale.shape[0]
        dev, dt = xpos.scale.device, xpos.scale.dtype
        qs, qc, qsc = [], [], []
        ks = torch.zeros(max_steps, max_steps, d, device=dev, dtype=dt)
        kc = torch.zeros(max_steps, max_steps, d, device=dev, dtype=dt)
        ksc = torch.zeros(max_steps, max_steps, d, device=dev, dtype=dt)
        for s in range(max_steps):
            sin, cos, scale = _xpos_tables(xpos, 1, s, False)
            qs.append(sin); qc.append(cos); qsc.append(scale)
            sin, cos, scale = _xpos_tables(xpos, s + 1, 0, True)
            ks[s, :s + 1] = sin; kc[s, :s + 1] = cos; ksc[s, :s + 1] = scale
        self._xq_sin = torch.stack(qs); self._xq_cos = torch.stack(qc); self._xq_scale = torch.stack(qsc)
        self._xk_sin = ks; self._xk_cos = kc; self._xk_scale = ksc

    # The compiled per-character step: tensors only (no ints), so dynamic=True yields a small,
    # stable set of graphs. Mirrors decoder_forward's op order exactly; the cached-buffer writes
    # move to the caller (same values, same destinations).
    def _step_math(self, tgt, cached_slice, memory, memory_mask,
                   q_sin, q_cos, q_scale, ks_sin, ks_cos, ks_scale, kc_sin, kc_cos, kc_scale):
        outs = []
        for l, layer in enumerate(self.decoders):
            combined = torch.cat([cached_slice[:, l], tgt], dim=1)
            outs.append(tgt)
            tgt = tgt + _attn_with_tables(layer.self_attn, layer.norm1(tgt), layer.norm1(combined), layer.norm1(combined),
                                          None, q_sin, q_cos, q_scale, ks_sin, ks_cos, ks_scale)
            tgt = tgt + _attn_with_tables(layer.multihead_attn, layer.norm2(tgt), memory, memory,
                                          memory_mask, q_sin, q_cos, q_scale, kc_sin, kc_cos, kc_scale)
            tgt = tgt + layer._ff_block(layer.norm3(tgt))
        outs.append(tgt)
        stacked = torch.cat(outs, dim=1)  # [B, n_layers+1, E] — row l = what decoder_forward wrote at [:, l, step]
        pred_char_logprob = self.pred(self.pred1(tgt.squeeze(1))).log_softmax(-1)
        return stacked, pred_char_logprob
    ...
    def infer_beam_batch_tensor(self, img: torch.FloatTensor, img_widths: List[int], beams_k: int = 5, start_tok = 1, end_tok = 2, pad_tok = 0, max_finished_hypos: int = 2, max_seq_length = 384):
        ...
        # EXPERIMENTAL (48px_exp): table-driven step — see _step_math. Tables are int-indexed
        # views (eager, free); the compiled fn sees only tensors. Cross-attn k tables depend on
        # this chunk's memory width, so compute them once here with the original formula.
        if not hasattr(self, '_xq_sin') or self._xq_sin.shape[0] < max_seq_length:
            self.build_xpos_tables(max(256, max_seq_length))
        step_fn = getattr(self, '_step_c', None) or self._step_math
        # Normalize memory's layout: it arrives as an einops permute view here but contiguous
        # after the later repeat_interleave/index_select — two stride families would double
        # every compiled-graph variant. Same values (plain copy), one layout.
        memory = memory.contiguous()
        kc_sin, kc_cos, kc_scale = _xpos_tables(self.decoders[0].self_attn.xpos, memory.size(1), 0, True)

        # Mark batch/memory-width dims dynamic up front (they're never 0/1) so the very first
        # graph is symbolic there — otherwise Dynamo pins them per family and a rare shape
        # combo (e.g. a 1-region chunk: B=5 with a wide memory) recompiles mid-gallery. The KV
        # length dim is left to automatic dynamics: it must 0/1-specialize on the first two
        # steps anyway, and warmup compiles those variants.
        _compiled = getattr(self, '_step_c', None) is not None
        def _mark_dyn(t, *dims):
            if _compiled:
                for d in dims:
                    torch._dynamo.mark_dynamic(t, d)
            return t

        stacked, pred_char_logprob = step_fn(_mark_dyn(idx_embedded, 0), _mark_dyn(cached_activations[:, :, :0, :], 0), _mark_dyn(memory, 0, 1), _mark_dyn(input_mask, 0, 1),
                                             self._xq_sin[0], self._xq_cos[0], self._xq_scale[0],
                                             self._xk_sin[0, :1], self._xk_cos[0, :1], self._xk_scale[0, :1],
                                             _mark_dyn(kc_sin, 0), _mark_dyn(kc_cos, 0), _mark_dyn(kc_scale, 0))
        cached_activations[:, :, 0, :] = stacked
        ...
        for step in range(1, max_seq_length):
            idx_embedded = self.embd(out_idx[:, -1:])
            stacked, pred_char_logprob = step_fn(_mark_dyn(idx_embedded, 0), _mark_dyn(cached_activations[:, :, :step, :], 0), _mark_dyn(memory, 0, 1), _mark_dyn(input_mask, 0, 1),
                                                 self._xq_sin[step], self._xq_cos[step], self._xq_scale[step],
                                                 self._xk_sin[step, :step + 1], self._xk_cos[step, :step + 1], self._xk_scale[step, :step + 1],
                                                 _mark_dyn(kc_sin, 0), _mark_dyn(kc_cos, 0), _mark_dyn(kc_scale, 0))
            cached_activations[:, :, step, :] = stacked
