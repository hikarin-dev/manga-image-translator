//! Faithful port of koharu's render driver (`koharu-app/src/renderer.rs`,
//! koharu v0.61.2, GPL-3.0) on top of the vendored, unmodified
//! `koharu-renderer` crate.
//!
//! Deviations from the original are marked `[shiori]` and are limited to:
//! - the Google Fonts service is removed (no network); fonts come from the
//!   system database plus explicitly registered font files,
//! - `NodeId` is a plain `u64` supplied by the caller,
//! - `RenderedBlock` additionally reports the fitted `font_size` (informational
//!   only; no rendering behavior depends on it),
//! - the API-only helpers `available_fonts` / `resolve_post_script_name` are
//!   dropped.
...
use crate::core_types::{
    FontPrediction, TextAlign, TextDirection, TextShaderEffect, TextStrokeStyle, TextStyle,
    Transform,
};
...
/// Per-block sprite output. `transform` becomes the sprite placement when the
/// renderer expanded the layout beyond the original bubble.
pub struct RenderedBlock {
    pub node_id: u64,
    pub sprite: DynamicImage,
    pub rendered_direction: TextDirection,
    pub expanded_transform: Option<Transform>,
    /// [shiori] Fitted font size in px, reported for downstream style hints.
    pub font_size: f32,
    /// [shiori] Colors the block was actually drawn with. `stroke_color`
    /// equals `text_color` (and `stroke_width` is 0) when no stroke was drawn,
    /// so consumers can treat the pair as a single always-valid fg/bg.
    pub text_color: [u8; 4],
...
impl Renderer {
    ...
    /// [shiori] Register a font file with the book (replaces the Google Fonts
    /// download path as the source of non-system fonts). Returns
    /// `(family_name, post_script_name)` so the caller can reference it in
    /// `PageRenderOptions::document_font`.
    pub fn register_font_file(&self, path: &str) -> Result<(String, String)> {
        ...
        Ok((family, face.post_script_name.clone()))
    }
    ...
    fn render_one(
        ...
        let mut render_candidate = |layout: &LayoutRun<'_>| -> Result<RenderedTextCandidate> {
            ...
            let stroke_color = resolved_stroke.as_ref().map(|s| s.color);
            let stroke_width = resolved_stroke.as_ref().map(|s| s.width_px).unwrap_or(0.0);
        ...
        if let Some((mask, bubble_id)) = bubble_mask.zip(resolved_box.bubble_id) {
            ...
            return Ok(Some(RenderedBlock {
                ...
                text_color: color,
                stroke_color: candidate.stroke_color.unwrap_or(color),
    ...
    fn select_font(&self, style: &TextStyle) -> Result<Font> {
        ...
        for candidate in &style.font_families {
            ...
            // [shiori] Steps 2 and 4 of the original resolution order queried
            // the Google Fonts download cache and are removed with it.
...
struct RenderedTextCandidate {
    image: RgbaImage,
    transform: Transform,
    /// [shiori] carried alongside so `RenderedBlock` can report them.
    font_size: f32,
...
fn core_align_to_renderer(a: TextAlign) -> RendererTextAlign {
    match a {
        TextAlign::Left => RendererTextAlign::Left,
        TextAlign::Center => RendererTextAlign::Center,
        TextAlign::Right => RendererTextAlign::Right,
    }
}
...
fn find_input(blocks: &[RenderBlockInput], id: u64) -> &RenderBlockInput {
    blocks
        .iter()
        .find(|b| b.node_id == id)
...
// ---------------------------------------------------------------------------
// Tests (ported unchanged from koharu-app where applicable)
// ---------------------------------------------------------------------------
